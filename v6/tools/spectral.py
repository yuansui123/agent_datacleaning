...
def compute_power_density_matrix_hilbert(
    ...
    dict with keys:
        "numeric_results": {
            ...
            "time_bins": array of time points (seconds),
            "freq_bands": dict of band definitions,
            "band_names": list of band names,
            "mean_band_power": dict of mean power per band,
    ...
    numeric_results = {
        "power_matrix": pdm_normalized.tolist(),
        "time_bins": time_clipped.tolist(),
        "freq_bands": freq_bands,
        "band_names": band_names,
        "mean_band_power": mean_band_power,
    ...
    if make_plot:
        ...
        # Print statistics
        print("\n" + "="*60)
        print(f"PDM Statistics ({normalize_mode} normalization)")
        print("="*60)
        print(f"Shape: {pdm_normalized.shape}")
        print(f"Time range: {time_clipped[0]:.2f}s to {time_clipped[-1]:.2f}s")
        print(f"Global - Min: {pdm_clipped.min():.6f}, Max: {pdm_clipped.max():.6f}, "
              f"Mean: {pdm_clipped.mean():.6f}")
        print("\nPer-band statistics:")
        for band_name in band_names:
            marker = " ⚠️ POWERLINE" if band_name == "powerline_60hz" else ""
            print(f"  {band_name:15s}: mean={mean_band_power[band_name]:.6f}, "
                  f"max={max_band_power[band_name]:.6f}{marker}")
        
        if powerline_contamination is not None:
            print(f"\n⚠️  Powerline contamination: {powerline_contamination*100:.2f}% of total power")
        print("="*60 + "\n")
...
def compute_power_density_matrix(
    ...
    numeric_results = {
        "power_matrix": power_matrix.tolist(),  # Convert to list for JSON serialization
        "time_bins": t.tolist(),
        "freq_bands": freq_bands,
        "band_names": band_names,
        "mean_band_power": mean_band_power,
